// (c) 30-10-2019 Alemorf aleksey.f.morozov@gmail.com
...
bool Png::create(uint32_t w, uint32_t h) {
    if (loaded) {
    ...
    png_size_t row_size = 4 * w;
    size_t s1 = sizeof(png_bytep) * height;
    size_t total = s1 + row_size * height;
    row_pointers = (png_bytep*)malloc(total);
    memset(row_pointers, 0x80, total);
    if (!row_pointers) {
