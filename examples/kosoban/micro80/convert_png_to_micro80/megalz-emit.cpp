...
ULONG emit_megalz(struct optchain *optch, ULONG actual_len) {
    ...
    // some checks
    if (!optch) {
        printf("megalz-emit.c:emit_megalz() - NULL passed!\n");
        return 0;
    }
    ...
    while ((position < actual_len) && success) {
        ...
        if (length == 0) {
            printf("megalz-emit.c:emit_megalz() - encountered stop-code in optimal chain before emitting all data!\n");
            return 0;
        ...
        INVALID_CODE_MEGALZ:
            printf("megalz-emit.c:emit_megalz() - invalid code: length=%d, displacement=%d\n", length, disp);
            return 0;
        }
...
ULONG emit_hrum(struct optchain *optch, ULONG actual_len) {
    ...
    // some checks
    if (!optch) {
        printf("megalz-emit.c:emit_hrum() - NULL passed!\n");
        return 0;
    }
    ...
    while ((position < actual_len) && success) {
        ...
        if (length == 0) {
            printf("megalz-emit.c:emit_hrum() - encountered stop-code in optimal chain before emitting all data!\n");
            return 0;
        } else if (length == 1)  // either copy-byte or len=1 code
        {
            if (disp == 0)  // copy-byte (%1<byte>)
        ...
        INVALID_CODE_HRUM:
            printf("megalz-emit.c:emit_hrum() - invalid code: length=%d, displacement=%d\n", length, disp);
            return 0;
        }
    ...
    if (length == EMIT_FILE_INIT) {
        ...
        while ((position + length) >= EMIT_FILEBUF_SIZE)  // if we have to flush buffer
        {
            ...
            if (EMIT_FILEBUF_SIZE != mhmt_fwrite(buffer, EMIT_FILEBUF_SIZE)) {
                printf("megalz-emit.c:emit_file() can't write to file!\n");
                return 0;
            }
        ...
        printf("megalz-emit.c:emit_file() encountered invalid arguments!\n");
        return 0;
    }
}
...
ULONG emit_byte(UBYTE byte, ULONG operation) {
    ...
    switch (operation) {
        ...
        case EMIT_BYTE_ADD:
            ...
            {
                printf("megalz-emit.c:emit_byte() buffer overflow!\n");
                return 0;
            }
        ...
        default:
            printf("megalz-emit.c:emit_byte() encountered invalid arguments!\n");
            return 0;
    }
}
...
ULONG emit_bits(ULONG msb_aligned_bits, LONG length) {
    ...
    } else if (length > 0)  // add bits
    {
        ...
        printf("megalz-emit.c:emit_bits() encountered invalid arguments!\n");
        return 0;
    }
}
