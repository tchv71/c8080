...
void make_lz_codes(OFFSET position, ULONG actual_len, UBYTE *hash, struct lzcode *codes) {
    ...
    {
        printf("megalz-lz.c:make_lz_codes() encountered too many entries in codes[] table. Fatal error.\n");
        exit(1);
    }
...
ULONG get_lz_price_megalz(OFFSET position, struct lzcode *lzcode) {
    ...
    INVALID_CODE_MEGALZ:
        printf("megalz-lz.c:get_lz_price_megalz(): Found invalid code length=%d, displacement=%d\n", length, disp);
        return 0;
    }
}
