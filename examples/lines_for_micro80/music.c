...
static const uint16_t music[] = {
  ITEM(F,8), ITEM(A,4), ITEM(C2,4), ITEM(A,4), ITEM(F,8), ITEM(C,4), ITEM(D,4), ITEM(E,4),
  ITEM(F,8), ITEM(A,4), ITEM(C2,4), ITEM(A,4), ITEM(G,8), ITEM(C,4), ITEM(D,4), ITEM(E,4), ITEM(E,8), ITEM(C,4), ITEM(D,4), ITEM(E,4), ITEM(F,16),
  ITEM(F,8), ITEM(A,4), ITEM(C2,4), ITEM(A,4), ITEM(G,8), ITEM(C,4), ITEM(D,4), ITEM(E,4), ITEM(E,8), ITEM(C,4), ITEM(D,4), ITEM(E,4), ITEM(F,16),
  0
};
...
void PlayMusic(void) {
    ...
    for (;;) {
      uint8_t s = *i; ++i;
      if(s==0) { while(!ReadKeyboard(true)); break; }
      if(ReadKeyboard(true)) break;
      Sound(s, *i); ++i;
      rand();
    }
}
